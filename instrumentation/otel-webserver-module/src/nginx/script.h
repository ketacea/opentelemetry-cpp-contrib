...
struct ScriptAttributeDeclaration {
  ScriptAttributeDeclaration(const char* attribute, const char* script) :
        attribute(ToNgxString(attribute)), script(ToNgxString(script)) {}
  ScriptAttributeDeclaration(ngx_str_t attribute, ngx_str_t script) : attribute(attribute), script(script) {}
